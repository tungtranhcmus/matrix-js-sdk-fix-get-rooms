"use strict";
import { getConversationsFilter } from 'matrix-js-sdk/lib/matrix';
...
class MatrixClient extends _typedEventEmitter.TypedEventEmitter {
  ...
  createFilter(content) {
    const path = utils.encodeUri("/user/$userId/filter", {
     ...
     return this.http.authedRequest(undefined, _httpApi.Method.Post, path, undefined,
      ...
      // persist the filter ok
      const newFilter = getConversationsFilter()
      const filter = _filter.Filter.fromJson(this.credentials.userId, newFilter, content);
  ...
  loginWithPassword(user, password, deviceName, callback) {
    // TODO: Types
